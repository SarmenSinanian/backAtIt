...
def initialize_db():
    db_exists = exists(
        r"C:\Users\Sarmen\PycharmProjects\HelloWorld1\2022_Programming\backAtIt\HackerRank2022\Day54(ShopifyImageRepository)\SQLite_Python.db")
    if not db_exists:
...
def addPhoto():
    ...
    for count, i in enumerate(pic, 1):
        ...
        if finalEntryID is None:
            finalEntryID = [0]
        # Convert data into tuple format
        data_tuple = (count+finalEntryID[0], 'Joe', photoBytes)
        sqlite_addPhoto_query = """ INSERT INTO new_employee
                                          (id, name, photo) VALUES ( ?, ?, ?)"""
        cursor.execute(sqlite_addPhoto_query, data_tuple)
        sqliteConnection.commit()
